...
|----------|---------|--------|---------------|
| `0x610` `MSR_PKG_POWER_LIMIT` | Hard package (CPU + iGPU uncore) power cap: sustained = PL1, burst = PL2 | Bounds CPU **and** integrated GPU power together, because the RAPL "package" domain includes the uncore. Directly changes sustained clocks, thermals and fan behaviour under load | Runtime only — lost on reboot; the script does not save the original value, so record the original MSR/sysfs state before applying a change if rollback is required |
...
**Write guards:** writes to `0x610` and `0x65C` are refused when bit 63 is set;
the `0x64B` cTDP-control write is refused when bit 31 is set.
...
Runtime MSR and powercap values reset on reboot, but the generated
`intel_lpmd` XML remains on disk. The generic XML has no automatic backup, and
model-specific files are backed up only once as `.orig`; restore those files
from `.orig` or an external/package backup, then restart `intel_lpmd`, rather
than assuming that rerunning a previous watt target restores the old state.
...
The interesting subset of the XML staged and installed:
...
Dry-run resolves the requested targets, performs available read-only probes,
calculates PL1/PL2/tau/EPP/EPB, prints the equivalent explicit-target command,
and exits before configuration installation, daemon restart, cTDP selection,
RAPL writes, or result verification. It does not apply any changes:
