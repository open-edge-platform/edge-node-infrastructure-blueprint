...
- `validate-platform-config`: Validate post-provision platform readiness locally on the provisioned host [lightweight Kubernetes distribution (k3s) pods, binaries and path, cloud-init, network, proxy values, devices, and GPU virtual functions (VFs)].
...
6. `Use validate-platform-config to verify this provisioned node locally and report checks for pods, k3s/kubectl binaries, cloud-init, networking, proxy values, devices, and GPU VFs.`
